...
using namespace std;
ll a[50010];struct seg {ll maxsum;ll prfsum;ll sufsum;ll totsum;};seg tree[200040];

void update(ll x,ll y,ll node,ll s,ll e)
{
	if(s==e)
	{
		tree[node].maxsum=y;tree[node].prfsum=y;tree[node].sufsum=y;tree[node].totsum=y;return;
	}
	ll mid=(s+e)/2;
	if(x<=mid) update(x,y,2*node,s,mid);
	else update(x,y,2*node+1,mid+1,e);
...
int main()
{
	...
	build(1,1,n);ll m;cin>>m;
	while(m--)
	{
		ll a,b,c;cin>>a>>b>>c;
		if(a)
		{
			seg ans=query(b,c,1,1,n);
			cout<<ans.maxsum<<"\n";
		}
